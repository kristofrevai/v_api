/**
 * E-mail küldés a Resend API-n keresztül - öt funkció:
 ...
 *
 * A boltvezetői e-mail (sima HTML, nem az emailShell-t használja, mert
 * belső, funkcionális levél) tartalmaz egy táblázatot a rendelt
 * tételekről, fölötte a megrendelő nevével. Emellett egy CSV fájl is
 * csatolásra kerül, ugyanazokkal az oszlopokkal.
 *
    ...
    return (
      "<tr>" +
        '<td style="padding:10px 0; font-family:' + FONT_BODY + '; font-size:13.5px; color:' + COLOR_TEXT + '; border-top:1px solid #f2f2f2;">' +
          escapeHtml(it.name) + '<br><span style="color:' + COLOR_MUTED + '; font-size:12px;">' + escapeHtml(it.unit) + '</span>' +
        "</td>" +
...
function buildWelcomeHtml(name, accountType) {
  ...
  const bodyRows =
    '<tr><td style="padding:32px 36px 6px 36px;">' +
      ...
      '<p style="font-family:' + FONT_BODY + '; font-size:13.5px; line-height:1.65; color:#666666; margin:0;">Köszönjük, hogy regisztrált a Révai Fruit Kft. webáruházában (' + escapeHtml(accountLabel) + ' fiókkal). Mostantól még kényelmesebben rendelhet nálunk.</p>' +
    "</td></tr>" +
